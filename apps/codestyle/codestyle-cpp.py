...
# Global variables
error_handler = False
findings = []  # (relative_path, line, message) collected for the CI review report
results = {
...
# annotation (::error) so the message shows up on the exact line in the PR diff.
def report(message: str, file_path: str, line: int = None) -> None:
    human = f"{message}: {file_path}" + (f" at line {line}" if line is not None else "")
    print(human)
    rel = os.path.relpath(file_path).replace(os.sep, "/")
    findings.append((rel, line, message))
    if IN_CI:
...
def write_report_file() -> None:
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        ...
        for rel, line, message in findings:
            ...
            f.write(f"- {loc} — {message}\n")
        f.write("\nSee the [C++ Code Standards](https://www.azerothcore.org/wiki/cpp-code-standards).\n")
...
def write_job_summary() -> None:
    ...
    with open(summary_path, 'a', encoding='utf-8') as summary:
        summary.write("## C++ Codestyle results\n\n")
...
def get_counter_check(file: io, file_path: str) -> None:
    ...
    for line_number, line in enumerate(file, start = 1):
        if 'ObjectGuid::GetCounter()' in line:
            report("Please use ObjectGuid::ToString().c_str() instead ObjectGuid::GetCounter()", file_path, line_number)
            if not error_handler:
                error_handler = True
...
def get_typeid_check(file: io, file_path: str) -> None:
    ...
    for line_number, line in enumerate(file, start = 1):
        if 'GetTypeId() == TYPEID_ITEM' in line or 'GetTypeId() != TYPEID_ITEM' in line:
            report("Please use IsItem() instead of GetTypeId()", file_path, line_number)
            check_failed = True
        if 'GetTypeId() == TYPEID_UNIT' in line or 'GetTypeId() != TYPEID_UNIT' in line:
            report("Please use IsCreature() instead of GetTypeId()", file_path, line_number)
            check_failed = True
        if 'GetTypeId() == TYPEID_PLAYER' in line or 'GetTypeId() != TYPEID_PLAYER' in line:
            report("Please use IsPlayer() instead of GetTypeId()", file_path, line_number)
            check_failed = True
        if 'GetTypeId() == TYPEID_GAMEOBJECT' in line or 'GetTypeId() != TYPEID_GAMEOBJECT' in line:
            report("Please use IsGameObject() instead of GetTypeId()", file_path, line_number)
            check_failed = True
        if 'GetTypeId() == TYPEID_DYNOBJECT' in line or 'GetTypeId() != TYPEID_DYNOBJECT' in line:
            report("Please use IsDynamicObject() instead of GetTypeId()", file_path, line_number)
            check_failed = True
    # Handle the script error and update the result output
    if check_failed:
        error_handler = True
...
def npcflags_helpers_check(file: io, file_path: str) -> None:
    ...
    for line_number, line in enumerate(file, start = 1):
        if 'GetUInt32Value(UNIT_NPC_FLAGS)' in line:
            report("Please use GetNpcFlags() instead of GetUInt32Value(UNIT_NPC_FLAGS)", file_path, line_number)
            check_failed = True
        if 'HasFlag(UNIT_NPC_FLAGS,' in line:
            report("Please use HasNpcFlag() instead of HasFlag(UNIT_NPC_FLAGS, ...)", file_path, line_number)
            check_failed = True
        if 'SetUInt32Value(UNIT_NPC_FLAGS,' in line:
            report("Please use ReplaceAllNpcFlags() instead of SetUInt32Value(UNIT_NPC_FLAGS, ...)", file_path, line_number)
            check_failed = True
        if 'SetFlag(UNIT_NPC_FLAGS,' in line:
            report("Please use SetNpcFlag() instead of SetFlag(UNIT_NPC_FLAGS, ...)", file_path, line_number)
            check_failed = True
        if 'RemoveFlag(UNIT_NPC_FLAGS,' in line:
            report("Please use RemoveNpcFlag() instead of RemoveFlag(UNIT_NPC_FLAGS, ...)", file_path, line_number)
            check_failed = True
    # Handle the script error and update the result output
    if check_failed:
        error_handler = True
...
def itemflag_helpers_check(file: io, file_path: str) -> None:
    ...
    for line_number, line in enumerate(file, start = 1):
        if 'HasFlag(ITEM_FIELD_FLAGS, ITEM_FIELD_FLAG_REFUNDABLE)' in line:
            report("Please use IsRefundable() instead of HasFlag(ITEM_FIELD_FLAGS, ITEM_FIELD_FLAG_REFUNDABLE)", file_path, line_number)
            check_failed = True
        if 'HasFlag(ITEM_FIELD_FLAGS, ITEM_FIELD_FLAG_BOP_TRADEABLE)' in line:
            report("Please use IsBOPTradable() instead of HasFlag(ITEM_FIELD_FLAGS, ITEM_FIELD_FLAG_BOP_TRADEABLE)", file_path, line_number)
            check_failed = True
        if 'HasFlag(ITEM_FIELD_FLAGS, ITEM_FIELD_FLAG_WRAPPED)' in line:
            report("Please use IsWrapped() instead of HasFlag(ITEM_FIELD_FLAGS, ITEM_FIELD_FLAG_WRAPPED)", file_path, line_number)
            check_failed = True
    # Handle the script error and update the result output
    if check_failed:
        error_handler = True
...
def itemtemplateflag_helpers_check(file: io, file_path: str) -> None:
    ...
    for line_number, line in enumerate(file, start = 1):
        if 'Flags & ITEM_FLAG' in line:
            report("Please use HasFlag(ItemFlag) instead of 'Flags & ITEM_FLAG_'", file_path, line_number)
            check_failed = True
        if 'Flags2 & ITEM_FLAG2' in line:
            report("Please use HasFlag2(ItemFlag2) instead of 'Flags2 & ITEM_FLAG2_'", file_path, line_number)
            check_failed = True
        if 'FlagsCu & ITEM_FLAGS_CU' in line:
            report("Please use HasFlagCu(ItemFlagsCustom) instead of 'FlagsCu & ITEM_FLAGS_CU_'", file_path, line_number)
            check_failed = True
    # Handle the script error and update the result output
    if check_failed:
        error_handler = True
...
def misc_codestyle_check(file: io, file_path: str) -> None:
    ...
    for line_number, line in enumerate(file, start = 1):
        if 'const auto&' in line:
            report("Please use the 'auto const&' syntax instead of 'const auto&'", file_path, line_number)
            check_failed = True
        if re.search(r'\bconst\s+\w+\s*\*\b', line):
            report("Please use the 'Class/ObjectType const*' syntax instead of 'const Class/ObjectType*'", file_path, line_number)
            check_failed = True
        if [match for match in [' if(', ' if ( '] if match in line]:
            report("Please use the 'if (XXXX)' syntax instead of 'if(XXXX)'", file_path, line_number)
            check_failed = True
        if re.match(ifelse_curlyregex, line):
            report("Curly brackets are not allowed to be leading or trailing if/else statements. Place it on a new line", file_path, line_number)
            check_failed = True
